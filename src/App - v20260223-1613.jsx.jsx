...
// ── Seed data ─────────────────────────────────────────────────────────────
const SEED=[
  {id:"s1",metricId:"vo2max",value:33.5,date:"2024-06-15",note:"Baseline"},
  {id:"s2",metricId:"vo2max",value:34.8,date:"2024-09-20"},
  {id:"s3",metricId:"vo2max",value:36.1,date:"2025-01-10"},
  {id:"s4",metricId:"vo2max",value:37.9,date:"2025-06-01",note:"Zone 2"},
  {id:"s5",metricId:"rhr",value:62,date:"2024-06-15",note:"Baseline"},
  {id:"s6",metricId:"rhr",value:58,date:"2024-09-20"},
  {id:"s7",metricId:"rhr",value:55,date:"2025-01-10"},
  {id:"s8",metricId:"rhr",value:51,date:"2025-06-01"},
  {id:"s9",metricId:"bp",value:128,secondary:82,date:"2024-06-15",note:"Baseline"},
  {id:"s10",metricId:"bp",value:122,secondary:79,date:"2024-09-20"},
  {id:"s11",metricId:"bp",value:118,secondary:76,date:"2025-01-10"},
  {id:"s12",metricId:"bp",value:112,secondary:74,date:"2025-06-01"},
  {id:"s13",metricId:"glucose",value:98,date:"2024-06-15",note:"Baseline"},
  {id:"s14",metricId:"glucose",value:93,date:"2024-09-20"},
  {id:"s15",metricId:"glucose",value:88,date:"2025-01-10"},
  {id:"s16",metricId:"glucose",value:82,date:"2025-06-01"},
  {id:"s17",metricId:"bodyfat",value:26.5,date:"2024-06-15",note:"Baseline"},
  {id:"s18",metricId:"bodyfat",value:24.2,date:"2024-09-20"},
  {id:"s19",metricId:"bodyfat",value:22.1,date:"2025-01-10"},
  {id:"s20",metricId:"bodyfat",value:20.4,date:"2025-06-01"},
];
...
async function parseAH(file,onProgress){
  if(file.name.toLowerCase().endsWith(".zip")&&!window.JSZip){
    await new Promise((res,rej)=>{const s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js";s.onload=res;s.onerror=()=>rej(new Error("Failed to load JSZip"));document.head.appendChild(s);});
  }
  ...
  if(file.name.toLowerCase().endsWith(".zip")){
    ...
    const xe=zip.file("apple_health_export/export.xml")||zip.file("export.xml");
    if(!xe)throw new Error("Could not find export.xml in ZIP.");
    await new Promise((resolve,reject)=>{
      let buf="",first=true;
      xe.internalStream("string")
        .on("data",(chunk,meta)=>{if(first){exMeta(chunk);first=false;}buf+=chunk;const le=proc(buf);const ti=buf.lastIndexOf("<Record",le);buf=(ti!==-1&&ti>le-32768)?buf.slice(ti):"";if(onProgress&&meta)onProgress(Math.min(99,Math.round(meta.percent||0)));})
        .on("end",()=>{if(buf)proc(buf);resolve();})
        .on("error",e=>reject(new Error(e.message)))
        .resume();
    });
  }else{const t=await file.text();exMeta(t);proc(t);}
...
function buildCanvas(entries,sex,eth,bioAge,chronoAge,days){
  const W=1500,H=940;const cv=document.createElement("canvas");cv.width=W;cv.height=H;
  const ctx=cv.getContext("2d");const now=new Date();
  const cutoff=new Date();cutoff.setDate(cutoff.getDate()-days);
  const cStr=cutoff.toISOString().substring(0,10);
  ...
  const pl=pLabel.toUpperCase();ctx.fillText(pl,W/2-ctx.measureText(pl).width/2,46);
  ...
  // cards
  const PAD=16,TOP=88,CW=Math.floor((W-PAD*4)/3),CH=Math.floor((H-TOP-PAD*2)/2)-2;
  const R2Y=TOP+CH+PAD,R2X=(W-2*CW-PAD)/2;
  MK.forEach((id,idx)=>{
    const col=idx%3,row=Math.floor(idx/3);
    const cx=row===0?PAD+col*(CW+PAD):R2X+(idx-3)*(CW+PAD);
    const cy=row===0?TOP:R2Y;
    drawCard(ctx,cx,cy,CW,CH,id,entries,sex,eth,cStr);
  });
  return cv;
}
function drawCard(ctx,x,y,w,h,id,entries,sex,eth,cStr){
  ...
  // sparkline
  const SX=PX,SY=y+116,SW=w-36,SH=h-210;
  if(period.length>1){
    const vs=period.map(e=>e.value);const mn=Math.min(...vs),mx=Math.max(...vs),vr=mx-mn||1;
    const tx=i=>SX+(i/(period.length-1))*SW;const ty=v=>SY+SH-((v-mn)/vr)*SH;
...
function SnapshotModal({entries,sex,eth,bioAge,chronoAge,onClose}){
  ...
  const render=useCallback(()=>{
    setRendering(true);
    setTimeout(()=>{
      try{const cv=buildCanvas(entries,sex,eth,bioAge,chronoAge,period);cvRef.current=cv;setImg(cv.toDataURL("image/jpeg",0.92));}
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{background:T.card,border:"1px solid #1a2a1a",borderRadius:16,width:"100%",maxWidth:800,maxHeight:"93vh",overflowY:"auto",padding:"26px 30px",position:"relative",animation:"fadeUp 0.2s ease"}}>
      ...
      <div style={{borderRadius:10,overflow:"hidden",border:"1px solid #0e1824",marginBottom:14,minHeight:200,display:"flex",alignItems:"center",justifyContent:"center",background:"#060a10"}}>
        {rendering?<div style={{color:T.dim,fontSize:13,fontFamily:T.fn}}><span style={{display:"inline-block",animation:"spin 1s linear infinite",marginRight:8}}>⟳</span>Rendering…</div>
          :img?<img src={img} alt="Snapshot" style={{width:"100%",display:"block"}}/>
          :<div style={{color:T.dim,fontSize:12}}>Preview unavailable</div>}
      </div>
...
export default function BioAgeTracker(){
  ...
  const bioTrend=[...new Set(entries.map(e=>e.date))].sort().slice(-14).map(d=>{
    const ba=getBioAge(entries.filter(e=>e.date<=d),age,sex,eth);
    return{date:d.substring(5),"Bio Age":ba?+ba.toFixed(1):null,Chrono:age};
  });
